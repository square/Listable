...
public enum Sizing: Hashable {
    ...
    func measure(with view: UIView, info: MeasureInfo) -> CGSize {
        ...
        validateMeasuredSize(size, with: info)
    ...
    private func validateMeasuredSize(_ size: CGSize, with info: MeasureInfo) {
        ...
        let reasonableMaxDimension: CGFloat = 5000
...
public extension Sizing {
    ...
    struct Constraint: Hashable {
        ...
        public func clamp(_ value: CGSize) -> CGSize {
            CGSize(
                width: width.clamp(value.width),
                height: height.clamp(value.height)
            )
        }
        ...
        public enum Axis: Hashable {
            ...
            public func clamp(_ value: CGFloat) -> CGFloat {
                ...
                case let .atLeast(minimum): return max(minimum, value)
                case let .atMost(maximum): return min(maximum, value)
                case let .within(minimum, maximum): return max(minimum, min(maximum, value))
                }
            }
        }
    }
