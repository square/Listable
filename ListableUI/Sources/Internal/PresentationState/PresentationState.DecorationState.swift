//
//  PresentationState.DecorationState.swift
//  ListableUI
//
...
protocol AnyPresentationDecorationState : AnyObject
{
    var isDisplayed : Bool { get }
    func setAndPerform(isDisplayed: Bool)
    
    var anyModel : AnyDecoration { get }
    ...
    func dequeueAndPrepareReusableDecorationView(
        in cache : ReusableViewCache,
        frame : CGRect,
        environment : ListEnvironment
    ) -> UIView
    
    func enqueueReusableDecorationView(_ view : UIView, in cache : ReusableViewCache)
    ...
    func set(
        ...
        info : ApplyDecorationContentInfo
    )
...
{
    final class DecorationViewStatePair
    {
        private(set) var state : AnyPresentationDecorationState?
        
        private(set) var visibleContainer : SupplementaryContainerView?
        
        init(state : AnyPresentationDecorationState?) {
            self.state = state
        }
        
        func update(
            with state : AnyPresentationDecorationState?,
            new: AnyDecorationConvertible?,
            reason: ApplyReason,
            animated : Bool,
            updateCallbacks : UpdateCallbacks,
            environment: ListEnvironment
            ...
            if self.state !== state {
                self.state = state
                fatalError("TODO")
                //self.visibleContainer?.setDecoration(state, animated: reason.shouldAnimate && animated)
            } else {
                if let state = state, let new = new {
                    state.set(
                        new: new.asAnyDecoration(),
                        reason: reason,
        ...
        func collectionViewDidEndDisplay(of view : SupplementaryContainerView)
        {
            ...
            ///
            /// Because we manage the `Decoration` view instances ourselves,
    ...
    final class DecorationState<Content:DecorationContent> : AnyPresentationDecorationState
    {
        var model : Decoration<Content>
        
        let performsContentCallbacks : Bool
                
        init(
            _ model : Decoration<Content>,
        ...
        // MARK: AnyPresentationDecorationState
        ...
        ) -> UIView
        {
            let view = cache.pop(with: self.model.reuseIdentifier) {
                DecorationContentView<Content>(frame: frame)
            }
        ...
        func applyTo(
            ...
            let view = view as! DecorationContentView<Content>
        ...
        func set(
            ...
            self.model = new as! Decoration<Content>
        ...
        ) -> CGSize
        {
            ...
            if let size = self.cachedSizes[key] {
                ...
                SignpostLogger.log(.begin, log: .updateContent, name: "Measure Decoration", for: self.model)
                
                let size : CGSize = cache.use(
                    with: self.model.reuseIdentifier,
                    create: {
                        return DecorationContentView<Content>(frame: .zero)
                }, { view in
                    let views = DecorationContentViews<Content>(view: view)
                ...
                SignpostLogger.log(.end, log: .updateContent, name: "Measure Decoration", for: self.model)
