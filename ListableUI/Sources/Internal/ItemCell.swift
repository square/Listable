...
final class ItemCell<Content: ItemContent>: UICollectionViewCell, AnyItemCell {
    ...
    override init(frame: CGRect) {
        ...
        contentContainer = ContentContainerView(frame: bounds)

        background = Content.createReusableBackgroundView(frame: bounds)
        selectedBackground = Content.createReusableSelectedBackgroundView(frame: bounds)
    ...
    override func sizeThatFits(_ size: CGSize) -> CGSize {
        contentContainer.contentView.sizeThatFits(size)
    }

    override func systemLayoutSizeFitting(_ targetSize: CGSize) -> CGSize {
        contentContainer.contentView.systemLayoutSizeFitting(targetSize)
    }
    ...
    override func systemLayoutSizeFitting(
        ...
        contentContainer.contentView.systemLayoutSizeFitting(
            targetSize,
    ...
    func closeSwipeActions() {
        contentContainer.performAnimatedClose()
    }
...
final class LiveCells {
    func add(_ cell: AnyItemCell) {
        cells.append(.init(cell: cell))

        cells = cells.filter { $0.cell != nil }
    }

    func perform(_ block: (AnyItemCell) -> Void) {
        cells.forEach {
            if let cell = $0.cell {
                block(cell)
            }
...
private extension UIView {
    func firstAccessibleChild() -> NSObject? {
