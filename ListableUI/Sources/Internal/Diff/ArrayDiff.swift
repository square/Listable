...
struct ArrayDiff<Element, Identifier: Hashable> {
    ...
    init(
        ...
        moved.sort { $0.new.newIndex > $1.new.newIndex }
        self.updated.sort { $0.newIndex < $1.newIndex }
        noChange.sort { $0.newIndex < $1.newIndex }

        changeCount = self.added.count
            + self.removed.count
            + moved.count
            + self.updated.count
    }
    ...
    private final class Pair {
        ...
        static func pairs(
            ...
            new.containers.map { newContainer in
...
extension ArrayDiff {
    func transform<Mapped>(
        old: [Mapped],
        removed: (Element, Mapped) -> Void,
        added: (Element) -> Mapped,
        moved: (Element, Element, inout Mapped) -> Void,
        updated: (Element, Element, inout Mapped) -> Void,
        noChange: (Element, Element, inout Mapped) -> Void
    ) -> [Mapped] {
        ...
        removes += self.moved.map {
            Removal(mapped: old[$0.old.oldIndex], removed: $0.old)
        }
    ...
    private final class Insertion<Mapped> {
        let mapped: Mapped
        let insert: Added
    ...
    private final class Removal<Mapped> {
        let mapped: Mapped
        let removed: Removed
...
private class DiffContainer<Value, Identifier: Hashable> {
    ...
    init(
        ...
        identifier = identifierFactory.identifier(for: identifierProvider(self.value))
    }
...
private struct UniqueIdentifier<Type, Identifier: Hashable>: Hashable {
    ...
    static func == (lhs: UniqueIdentifier, rhs: UniqueIdentifier) -> Bool {
        lhs.hash == rhs.hash && lhs.base == rhs.base && lhs.modifier == rhs.modifier
    }
    ...
    final class Factory {
        ...
        func reserveCapacity(_ minimumCapacity: Int) {
            counts.reserveCapacity(minimumCapacity)
        }

        func identifier(for base: Identifier) -> UniqueIdentifier {
            let count = counts[base, default: 1]
...
private final class DiffableCollection<Value, Identifier: Hashable> {
    ...
    init(elements: [Value], _ identifierProvider: (Value) -> Identifier) {
        uniqueIdentifierIndexes.reserveCapacity(elements.count)

        containers = DiffContainer.containers(with: elements, identifierProvider: identifierProvider)

        containersByIdentifier = containers.toUniqueDictionary { _, container in
    ...
    func move(from: Int, to: Int) {
        ...
        let value = containers[from]

        containers.remove(at: from)
        containers.insert(value, at: to)
    ...
    private func resetIndexLookups() {
        uniqueIdentifierIndexes.removeAll(keepingCapacity: true)
    }

    private func generateIndexLookupsIfNeeded() {
        guard uniqueIdentifierIndexes.isEmpty else {
            return
        }

        containers.forEachWithIndex { index, _, container in
...
private extension Array {
    func toUniqueDictionary<Key: Hashable, Value>(_ block: (Int, Element) -> (Key, Value)) -> [Key: Value]
    {
        ...
        for (index, element) in enumerated() {
            ...
            guard dictionary[key] == nil else {
                listableInternalFatal("Existing entry for key '\(key)' not allowed for unique dictionaries.")
            }
