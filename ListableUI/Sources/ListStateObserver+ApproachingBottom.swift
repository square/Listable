...
{
    public typealias OnApproachingBottom = (ApproachingBottom) -> ()
    ...
    /// returns `true`.
    public mutating func onApproachingBottom(
    ...
    )
    {
        ...
        self.onDidScroll(observer.didScroll)
        self.onVisibilityChanged(observer.visibilityChanged)
        self.onContentUpdated(observer.contentUpdated)
        self.onFrameChanged(observer.frameChanged)
    }

    /// Parameters available for ``OnApproachingBottom`` callbacks.
    public struct ApproachingBottom {
