...
public struct Content {
    ...
    public var nonEmptySections: [Section] {
        sections.filter { $0.items.isEmpty == false }
    }
    ...
    public typealias Configure = (inout Content) -> Void
    ...
    public var firstItem: AnyItem? {
        guard let first = nonEmptySections.first?.items.first else {
            return nil
        }
    ...
    public var lastItem: AnyItem? {
        guard let last = nonEmptySections.last?.items.last else {
            return nil
        }
    ...
    internal func sliceTo(indexPath: IndexPath, plus additionalItems: Int = Content.Slice.defaultCount) -> Slice
    {
        ...
        sliced.sections = sections.compactMapWithIndex { sectionIndex, _, section in
        ...
        return Slice(
            containsAllItems: itemCount == sliced.itemCount,
            content: sliced
        )
    }
}
