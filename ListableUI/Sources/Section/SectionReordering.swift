...
public struct SectionReordering {
    ...
    func canReorderIn(with result: ItemReordering.Result) -> Bool {
        if let max = maxItemCount, max < (result.toSection.count + 1) {
            return false
        }

        return result.allowed(with: canReorderIn)
    }

    func canReorderOut(with result: ItemReordering.Result) -> Bool {
        if minItemCount > (result.fromSection.count - 1) {
            return false
        }

        return result.allowed(with: canReorderOut)
    }
}
