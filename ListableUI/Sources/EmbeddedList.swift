...
public struct EmbeddedList: ItemContent {
    ...
    public init<Identifier: Hashable>(identifier: Identifier, configure: ListProperties.Configure) {
        contentIdentifier = AnyHashable(identifier)
    ...
    public func apply(to views: ItemContentViews<Self>, for _: ApplyReason, with _: ApplyItemContentInfo)
    {
        views.content.configure(with: properties)
    }

    public func isEquivalent(to _: EmbeddedList) -> Bool {
        false
    }
