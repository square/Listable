...
public extension LayoutDescription {
    static func table(_ configure: (inout TableAppearance) -> Void = { _ in }) -> Self {
...
public struct TableAppearance: ListLayoutAppearance {
    ...
    public var scrollViewProperties: ListLayoutScrollViewProperties {
        .init(
            ...
            allowsBounceVertical: bounceOnUnderflow,
            allowsBounceHorizontal: bounceOnUnderflow,
...
public extension TableAppearance {
    struct ItemLayout: Equatable, ItemLayoutsValue {
    ...
    struct HeaderFooterLayout: Equatable, HeaderFooterLayoutsValue {
        public var width: CustomWidth
    ...
    struct SectionLayout: Equatable, SectionLayoutsValue {
        public var width: CustomWidth
        ...
        public struct Columns: Equatable {
            ...
            func group<Value>(values: [Value]) -> [[Value]] {
                ...
                while values.count > 0 {
                    grouped.append(values.safeDropFirst(count))
                }
    ...
    struct Layout: Equatable {
        ...
        public mutating func set(with block: (inout Layout) -> Void) {
            var edited = self
            block(&edited)
            self = edited
        }
...
final class TableListLayout: ListLayout {
    ...
    private func layout(
        ...
        after: (ListLayoutContent.SupplementaryItemInfo) -> Void
    ) {
    ...
    func layout(
        ...
        let bounds = layoutAppearance.bounds ?? context.environment.listContentBounds(in: boundsContext)

        let layout = layoutAppearance.layout

        let viewWidth = direction.width(for: context.viewBounds.size)
        ...
        self.layout(
            headerFooter: content.containerHeader,
            width: content.containerHeader.layouts.table.width.merge(with: .fill),
            viewWidth: viewWidth,
            defaultWidth: defaultWidth,
        ...
        contentBottom += direction.switch(
        ...
        self.layout(
            headerFooter: content.header,
            width: content.header.layouts.table.width.merge(with: rootWidth),
            viewWidth: viewWidth,
            defaultWidth: defaultWidth,
            contentBottom: contentBottom,
            after: { headerFooter in
        ...
        content.sections.forEachWithIndex { _, isLast, section in
            ...
            if section.layouts.table.columns.count == 1 {
                section.items.forEachWithIndex { _, isLast, item in
        ...
        self.layout(
            headerFooter: content.footer,
            width: content.footer.layouts.table.width.merge(with: rootWidth),
            viewWidth: viewWidth,
            defaultWidth: defaultWidth,
            contentBottom: contentBottom,
            after: { footer in
        ...
        self.layout(
            headerFooter: content.overscrollFooter,
            width: content.overscrollFooter.layouts.table.width.merge(with: rootWidth),
            viewWidth: viewWidth,
            defaultWidth: defaultWidth,
            contentBottom: contentBottom,
            after: { _ in }
...
private extension ListLayoutContent.SectionInfo {
    func setItemPositions(with appearance: TableAppearance) {
        if layouts.table.columns.count == 1 {
            ...
            items.forEach { $0.position = .single }
        }
    }
    ...
    private static func grouped(items: [ListLayoutContent.ItemInfo], groupingHeight: CGFloat, appearance: TableAppearance) -> [[ListLayoutContent.ItemInfo]]
    {
        ...
        items.forEachWithIndex { _, _, item in
...
private extension Array {
    mutating func safeDropFirst(_ count: Int) -> [Element] {
        ...
        removeFirst(safeCount)

        return Array(values)
    }
}

private func performLayout<Input>(for input: Input, _ block: (Input) -> Void) {
    block(input)
}
