...
public extension LayoutDescription {
    ...
    static func flow(
        _ configure: (inout FlowAppearance) -> Void = { _ in }
    ) -> Self {
...
public extension FlowAppearance {
    ...
    enum RowUnderflowAlignment: Equatable {
    ...
    enum RowItemsAlignment: Equatable {
...
public extension ItemLayouts {
    ...
    var flow: FlowAppearance.ItemLayout {
...
public extension HeaderFooterLayouts {
    ...
    var flow: FlowAppearance.HeaderFooterLayout {
...
public extension SectionLayouts {
    ...
    var flow: FlowAppearance.SectionLayout {
...
final class FlowListLayout: ListLayout {
    ...
    func layout(
        ...
        let spacings = layoutAppearance.spacings
        ...
        content.sections.forEachWithIndex { _, isLast, section in
            ...
            rows.forEachWithIndex { _, isLast, row in
    ...
    private func setX(
        ...
        case .leading, .centered, .trailing:
            ...
            row.items.forEachWithIndex { _, isLast, item in
                direction.switch {
                    item.x = maxX
                } horizontal: {
                    item.y = maxX
        ...
        case let .fill(lastRowAlignment):
    ...
    private func layout(
        ...
        let measureInfo = Sizing.MeasureInfo(
            sizeConstraint: direction.size(for: CGSize(width: position.width, height: .greatestFiniteMagnitude)),
            direction: direction
        )
...
extension CGFloat {
    ...
    func sliceIntoSpacings<Element>(
        with items: [Element],
        using block: (SliceSpacing, Element) -> Void
    ) {
        if items.isEmpty { return }

        let spacings = sliceIntoSpacings(for: items.count)
