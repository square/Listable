...
public extension LayoutDescription {
    static func retailGrid(_ configure: (inout RetailGridAppearance) -> Void = { _ in }) -> Self {
...
public struct RetailGridAppearance: ListLayoutAppearance {
    ...
    public var scrollViewProperties: ListLayoutScrollViewProperties {
        .init(
            isPagingEnabled: layout.isPaged,
    ...
    public struct Layout: Equatable {
        ...
        public init(
            ...
            if case let .rows(rowCount) = rows {
...
public extension RetailGridAppearance {
    struct ItemLayout: ItemLayoutsValue {
        ...
        func pageIndex(rows: Layout.Rows) -> Int {
            guard case let .rows(rowCount) = rows else {
                return 0
            }
        ...
        func frame(pageSize: CGSize, pagePadding: UIEdgeInsets, itemSpacing: CGFloat, columns: Int, rows: Layout.Rows) -> CGRect {
            let pageIndex = pageIndex(rows: rows)
            ...
            let realX = CGFloat(origin.x) * oneByOneTile.width + itemSpacing * CGFloat(origin.x)
...
public extension ItemLayouts {
    var retailGrid: RetailGridAppearance.ItemLayout {
...
final class RetailGridListLayout: ListLayout {
    ...
    func layout(
        ...
        if layoutAppearance.layout.isPaged {
