...
public struct LocalizedItemCollator {
    ...
    public static func sections(
        ...
        _ modify: (CollatedSection, inout Section) -> Void = { _, _ in }
    ) -> [Section] {
    ...
    private final class Provider {
        ...
        init(item: AnyLocalizedCollatableItem) {
            self.item = item
            collationString = self.item.collationString
        }
    }
}

public extension LocalizedItemCollator {
    /// The output of the collator, with the collated title and items
    /// that should be added to a given section.
    struct CollatedSection {
