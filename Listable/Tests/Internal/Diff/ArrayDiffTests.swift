...
class ArrayDiffTests: XCTestCase {
  ...
  func test_insert_and_remove() {
    self.testcase("Empty to filled") {
      ...
      let diff = ArrayDiff(
        old: old, new: new, identifierProvider: { AnyHashable($0) }, movedHint: { _, _ in false },
  ...
  func test_move_and_update() {
    ...
    self.testcase("Update items") {
      ...
      let diff = ArrayDiff(
        old: old, new: new, identifierProvider: { AnyHashable($0.lowercased()) },
        movedHint: { _, _ in false }, updated: { $0 != $1 })
...
extension Array {
  @discardableResult
  fileprivate mutating func removeRandom<RNG: RandomNumberGenerator>(using rng: inout RNG)
    -> Element
  {
    return self.remove(at: Int.random(in: 0..<self.count, using: &rng))
  }

  fileprivate mutating func insert<RNG: RandomNumberGenerator>(
    atRandom element: Element, using rng: inout RNG
  ) {
