...
class SupplementaryContainerViewTests: XCTestCase {
  func newHeaderFooter() -> AnyPresentationHeaderFooterState {
    let headerFooter = HeaderFooter(
      with: TestHeaderFooterElement(), appearance: TestHeaderFooterElement.Appearance())
    return PresentationState.HeaderFooterState(headerFooter)
  }

  func test_init() {
    let view = SupplementaryContainerView(
  ...
  func test_headerFooter() {
    ...
    XCTAssertEqual(
      cache.count(for: ReuseIdentifier.identifier(for: TestHeaderFooterElement.self)), 1)
    ...
    XCTAssertEqual(
      cache.count(for: ReuseIdentifier.identifier(for: TestHeaderFooterElement.self)), 0)
  }
