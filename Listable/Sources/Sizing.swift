...
public enum Sizing: Equatable {
  ...
  public func measure(
    with view: UIView, width: CGFloat, layoutDirection: LayoutDirection, defaultHeight: CGFloat
  ) -> CGFloat {
    let value: CGFloat = {
      switch self {
      case .default:
      ...
      case .thatFits:
        return Sizing.thatFitsWith(.noConstraint).measure(
          with: view, width: width, layoutDirection: layoutDirection, defaultHeight: defaultHeight)
  ...
  public enum Constraint: Equatable {
    ...
    public func clamp(_ value: CGFloat, with defaultHeight: CGFloat) -> CGFloat {
      ...
      case .within(let minimum, let maximum):
        return max(minimum.value(with: defaultHeight), min(maximum, value))
      }
    }
  }
}
...
public enum CustomWidth: Equatable {
  ...
  public func position(
    with viewSize: CGSize, defaultWidth: CGFloat, layoutDirection: LayoutDirection
  ) -> Position {
    switch self {
    case .default:
      return Position(
        origin: round((layoutDirection.width(for: viewSize) - defaultWidth) / 2.0),
        width: defaultWidth)
    case .fill: return Position(origin: 0.0, width: layoutDirection.width(for: viewSize))
    case .custom(let custom):
      return custom.position(with: viewSize, layoutDirection: layoutDirection)
    }
  }
  ...
  public enum Alignment: Equatable {
    ...
    public func originWith(
      parentWidth: CGFloat, width: CGFloat, padding: HorizontalPadding,
      layoutDirection: LayoutDirection
    ) -> CGFloat {
      switch self {
      case .left:
