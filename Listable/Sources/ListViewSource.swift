...
extension ListViewSource {
  public func content(with state: SourceState<State>) -> Content {
...
internal final class ReloadTimer {
  ...
  init(onFire: @escaping OnFire) {
    ...
    self.timer = Timer.scheduledTimer(
      timeInterval: 0.0, target: self, selector: #selector(timerFired), userInfo: nil,
      repeats: false)
  }
