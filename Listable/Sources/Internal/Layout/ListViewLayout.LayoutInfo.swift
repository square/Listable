...
extension ListViewLayout {
  final class LayoutInfo {
    ...
    init() {
      ...
      self.overscrollFooter = SupplementaryItemLayoutInfo.empty(
        .overscrollFooter, direction: appearance.direction)
    ...
    init(
      ...
      self.overscrollFooter = {
        guard delegate.hasOverscrollFooter(in: collectionView) else {
          return SupplementaryItemLayoutInfo.empty(
            .overscrollFooter, direction: appearance.direction)
        }
        ...
        return SectionLayoutInfo(
          ...
          header: {
            guard delegate.hasHeader(in: sectionIndex, in: collectionView) else {
              return SupplementaryItemLayoutInfo.empty(
                .sectionHeader, direction: appearance.direction)
            }
          ...
          footer: {
            guard delegate.hasFooter(in: sectionIndex, in: collectionView) else {
              return SupplementaryItemLayoutInfo.empty(
                .sectionFooter, direction: appearance.direction)
            }
    ...
    func supplementaryLayoutAttributes(of kind: String, at indexPath: IndexPath)
    ...
    func updateOverscrollPosition(in collectionView: UICollectionView) -> Bool {
      ...
      if contentHeight >= viewHeight {
        footer.y =
          contentHeight + direction.bottom(with: collectionView.contentInset)
          + direction.bottom(with: collectionView.lst_safeAreaInsets)
      } else {
        footer.y =
          viewHeight - direction.top(with: collectionView.contentInset)
          - direction.top(with: collectionView.lst_safeAreaInsets)
      }
    ...
    func layout(
      ...
      performLayout(for: self.header) { header in
        ...
        let position = header.layout.width.position(
          with: viewSize, defaultWidth: rootWidth, layoutDirection: direction)
        let height =
          hasListHeader
          ? delegate.heightForListHeader(
            in: collectionView, width: position.width, layoutDirection: direction) : 0.0
      ...
      self.sections.forEachWithIndex { sectionIndex, isLast, section in
        ...
        let sectionPosition = section.layout.width.position(
          with: viewSize, defaultWidth: rootWidth, layoutDirection: direction)
        ...
        performLayout(for: section.header) { header in
          ...
          let position = width.position(
            with: viewSize, defaultWidth: sectionPosition.width, layoutDirection: direction)
          let height =
            hasSectionHeader
            ? delegate.heightForHeader(
              in: sectionIndex, in: collectionView, width: position.width,
              layoutDirection: direction) : 0.0
        ...
        if section.columns.count == 1 {
          section.items.forEachWithIndex { itemIndex, isLast, item in
            ...
            let itemPosition = width.position(
              with: viewSize, defaultWidth: sectionPosition.width, layoutDirection: direction)
            let height = delegate.heightForItem(
              at: indexPath, in: collectionView, width: itemPosition.width,
            ...
            if isLast {
              if hasSectionFooter {
                lastContentMaxY +=
                  item.layout.itemToSectionFooterSpacing ?? layout.itemToSectionFooterSpacing
              }
            } else {
          ...
          let itemWidth = round(
            (sectionPosition.width - (section.columns.spacing * CGFloat(section.columns.count - 1)))
              / CGFloat(section.columns.count))
          ...
          groupedItems.forEachWithIndex { rowIndex, isLast, row in
            ...
            row.forEachWithIndex { columnIndex, isLast, item in
              ...
              let height = delegate.heightForItem(
                at: indexPath, in: collectionView, width: itemWidth, layoutDirection: direction)

              let itemSpacing = item.value.layout.itemSpacing ?? layout.itemSpacing
              let itemToSectionFooterSpacing =
                item.value.layout.itemToSectionFooterSpacing ?? layout.itemToSectionFooterSpacing

              item.value.size = direction.size(width: itemWidth, height: height)

              maxHeight = max(height, maxHeight)
              maxItemSpacing = max(itemSpacing, maxItemSpacing)
              maxItemToSectionFooterSpacing = max(
                itemToSectionFooterSpacing, maxItemToSectionFooterSpacing)
        ...
        if isLast == false {
          let additionalSectionSpacing =
            hasSectionFooter
            ? layout.interSectionSpacingWithFooter : layout.interSectionSpacingWithNoFooter
      ...
      performLayout(for: self.footer) { footer in
        ...
        let position = footer.layout.width.position(
          with: viewSize, defaultWidth: rootWidth, layoutDirection: direction)
        let height =
          hasFooter
          ? delegate.heightForListFooter(
            in: collectionView, width: position.width, layoutDirection: direction) : 0.0
      ...
      performLayout(for: self.overscrollFooter) { footer in
        ...
        let height =
          hasFooter
          ? delegate.heightForOverscrollFooter(
            in: collectionView, width: position.width, layoutDirection: direction) : 0.0
      ...
      self.adjustPositionsForLayoutUnderflow(
        contentHeight: lastContentMaxY, viewHeight: viewHeight, in: collectionView)
    ...
    private func adjustPositionsForLayoutUnderflow(
      contentHeight: CGFloat, viewHeight: CGFloat, in collectionView: UICollectionView
    ) {
      ...
      let safeAreaInsets: CGFloat = {
        switch self.appearance.direction {
        case .vertical:
          return collectionView.lst_safeAreaInsets.top + collectionView.lst_safeAreaInsets.bottom
        case .horizontal:
          return collectionView.lst_safeAreaInsets.left + collectionView.lst_safeAreaInsets.right
        }
      }()
...
extension ListViewLayout.LayoutInfo {
  ...
  final class SectionLayoutInfo {
    ...
    private static func grouped(
      items: [ItemLayoutInfo], groupingHeight: CGFloat, appearance: Appearance
    ) -> [[ItemLayoutInfo]] {
  ...
  final class SupplementaryItemLayoutInfo {
    static func empty(_ kind: ListViewLayout.SupplementaryKind, direction: LayoutDirection)
      -> SupplementaryItemLayoutInfo
    {
      return SupplementaryItemLayoutInfo(
        kind: kind, direction: direction, layout: .init(), isPopulated: false)
    }
    ...
    init(
      kind: ListViewLayout.SupplementaryKind, direction: LayoutDirection,
      layout: HeaderFooterLayout, isPopulated: Bool
    ) {
      self.kind = kind
      self.direction = direction
      self.layout = layout
    ...
    func layoutAttributes(with indexPath: IndexPath) -> UICollectionViewLayoutAttributes {
      let attributes = UICollectionViewLayoutAttributes(
        forSupplementaryViewOfKind: self.kind.rawValue, with: indexPath)
...
extension Array {
  fileprivate mutating func safeDropFirst(_ count: Int) -> [Element] {
...
extension Int {
  fileprivate func mapEach<Mapped>(_ block: (Int) -> Mapped) -> [Mapped] {
    var mapped = [Mapped]()
...
fileprivate func performLayout<Input>(for input: Input, _ block: (Input) -> Void) {
  block(input)
}
