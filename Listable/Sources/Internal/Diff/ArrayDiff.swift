...
private class DiffContainer<Value> {
  ...
  static func containers(with elements: [Value], identifierProvider: (Value) -> AnyHashable)
    -> [DiffContainer]
  {
...
private final class DiffableCollection<Value> {
  ...
  init(elements: [Value], _ identifierProvider: (Value) -> AnyHashable) {
    ...
    self.containers = DiffContainer.containers(
      with: elements, identifierProvider: identifierProvider)
...
extension Array {
  fileprivate func separate(_ block: (Element) -> Bool) -> ([Element], [Element]) {
    var left = [Element]()
    var right = [Element]()
...
extension Array {
  fileprivate func toUniqueDictionary<Key: Hashable, Value>(_ block: (Int, Element) -> (Key, Value))
    -> [Key: Value]
  {
