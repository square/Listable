...
struct SectionedDiff<Section, Item> {
  ...
  static func calculate(
    on queue: DispatchQueue, old: [Section], new: [Section], configuration: Configuration,
    completion: @escaping (SectionedDiff) -> Void
  ) {
    queue.async {
  ...
  struct SectionChanges {
    ...
    init(old: [Section], new: [Section], configuration: Configuration) {
      ...
      listablePrecondition(
        diff.updated.isEmpty, "Must not have any updates for sections; sections can only move.")
    }
...
extension SectionedDiff.SectionChanges.NoChange: Equatable
where Section: Equatable, Item: Equatable {}
