...
protocol AnyPresentationItemState: AnyObject {
  ...
  func dequeueAndPrepareCollectionViewCell(
    in collectionView: UICollectionView, for indexPath: IndexPath
  ) -> UICollectionViewCell

  func applyTo(
    cell anyCell: UICollectionViewCell, itemState: Listable.ItemState, reason: ApplyReason)
  func applyToVisibleCell()

  func setNew(item anyItem: AnyItem, reason: UpdateReason)

  func willDisplay(
    cell: UICollectionViewCell, in collectionView: UICollectionView, for indexPath: IndexPath)
  func didEndDisplay()
  ...
  func height(
    width: CGFloat, layoutDirection: LayoutDirection, defaultHeight: CGFloat,
    measurementCache: ReusableViewCache
  ) -> CGFloat
...
protocol AnyPresentationHeaderFooterState: AnyObject {
  ...
  func dequeueAndPrepareReusableHeaderFooterView(in cache: ReusableViewCache, frame: CGRect)
    -> UIView
...
final class PresentationState {
  ...
  public var lastIndexPath: IndexPath? {
    let nonEmptySections: [(index: Int, section: SectionState)] = self.sections.compactMapWithIndex
    { index, _, state in
  ...
  func update(with diff: SectionedDiff<Section, AnyItem>, slice: Content.Slice) {
    ...
    self.header.state = SectionState.headerFooterState(
      with: self.header.state, new: slice.content.header)
    self.footer.state = SectionState.headerFooterState(
      with: self.footer.state, new: slice.content.footer)
    self.overscrollFooter.state = SectionState.headerFooterState(
      with: self.overscrollFooter.state, new: slice.content.overscrollFooter)
    ...
    self.sections = diff.changes.transform(
      ...
      moved: { old, new, changes, section in
        section.update(with: old, new: new, changes: changes, listView: self.view)
      },
  ...
  final class SectionState {
    ...
    fileprivate func update(
      ...
      self.header.state = SectionState.headerFooterState(
        with: self.header.state, new: self.model.header)
      self.footer.state = SectionState.headerFooterState(
        with: self.footer.state, new: self.model.footer)
    ...
    fileprivate static func headerFooterState(
      with current: AnyPresentationHeaderFooterState?, new: AnyHeaderFooter?
      ...
      if let height = self.cachedHeights[heightKey] {
        ...
        let height: CGFloat = measurementCache.use(
          ...
          { view in
            ...
            return self.model.sizing.measure(
              with: view, width: width, layoutDirection: layoutDirection,
  ...
  final class ItemState<Element: ItemElement>: AnyPresentationItemState {
    ...
    func dequeueAndPrepareCollectionViewCell(
      ...
      let anyCell = collectionView.dequeueReusableCell(
        withReuseIdentifier: self.cellRegistrationInfo.reuseIdentifier, for: indexPath)
    ...
    func willDisplay(
      cell anyCell: UICollectionViewCell, in collectionView: UICollectionView,
      ...
      if let height = self.cachedHeights[heightKey] {
        ...
        let height: CGFloat = measurementCache.use(
          ...
          { cell in
            ...
            return self.model.sizing.measure(
              with: cell, width: width, layoutDirection: layoutDirection,
              defaultHeight: defaultHeight)
          })
