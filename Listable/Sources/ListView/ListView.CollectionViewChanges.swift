...
extension ListView {
  struct CollectionViewChanges {
    ...
    var hasIndexAffectingChanges: Bool {
      return
        self.deletedSections.isEmpty == false || self.insertedItems.isEmpty == false
        || self.movedSections.isEmpty == false || self.deletedItems.isEmpty == false
        || self.insertedItems.isEmpty == false || self.movedItems.isEmpty == false
    }
