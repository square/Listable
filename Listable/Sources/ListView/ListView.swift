...
public final class ListView: UIView {
  ...
  public init(frame: CGRect = .zero, appearance: Appearance = Appearance()) {
    ...
    self.collectionView = UICollectionView(
      frame: CGRect(origin: .zero, size: frame.size), collectionViewLayout: self.layout)
  ...
  private func applyAppearance() {
    ...
    self.collectionView.setAlwaysBounce(
      self.appearance.underflow.alwaysBounce, direction: self.appearance.direction)
  }
  ...
  @discardableResult
  public func scrollTo<Element: ItemElement>(
    item: Identifier<Element>, position: ItemScrollPosition, animated: Bool = false
  ) -> Bool {
    return self.scrollTo(item: AnyIdentifier(item), position: position, animated: animated)
  }

  @discardableResult
  public func scrollTo(item: AnyIdentifier, position: ItemScrollPosition, animated: Bool = false)
    -> Bool
  {
  ...
  public func scrollToBottom(animated: Bool = false) -> Bool {
    ...
    return self.preparePresentationStateForScroll(to: toIndexPath) {
      ...
      let contentOffsetY =
        contentHeight - contentFrameHeight - self.collectionView.lst_adjustedContentInset.top
  ...
  @discardableResult
  public func set<Source: ListViewSource>(
    source: Source, initial: Source.State, animated: Bool = false
  ) -> StateAccessor<Source.State> {
    self.sourcePresenter.discard()

    let sourcePresenter = SourcePresenter(
  ...
  private func setContentFromSource(animated: Bool = false) {
    ...
    self.updatePresentationState(
      for: .contentChanged(animated: animated, identifierChanged: identifierChanged))
  }
  ...
  internal func updateVisibleItemsAndSections() {
    ...
    let newVisibleItems = Set(
      visibleIndexPaths.map {
        VisibleItem(item: self.storage.presentationState.item(at: $0))
      })

    let visibleSectionIndexes: Set<Int> = visibleIndexPaths.reduce(
      into: Set(), { $0.insert($1.section) })
  ...
  internal func updatePresentationState(
    for reason: Content.Slice.UpdateReason, completion: @escaping (Bool) -> Void = { _ in }
  ) {
    ...
    case .scrolledDown:
      ...
      if needsUpdate {
        self.updatePresentationStateWith(
          firstVisibleIndexPath: indexPath, for: reason, completion: completion)
      } else {
        completion(true)
      }
    ...
    case .scrolledToTop:
      if presentationStateTruncated {
        self.updatePresentationStateWith(
          firstVisibleIndexPath: IndexPath(item: 0, section: 0), for: reason, completion: completion
        )
      } else {
        completion(true)
      }
    ...
    case .programaticScrollDownTo(let scrollToIndexPath):
      self.updatePresentationStateWith(
        firstVisibleIndexPath: scrollToIndexPath, for: reason, completion: completion)
    }
  }
  ...
  private func updatePresentationStateWith(
    ...
    if self.bounds.isEmpty {
      ...
      case .scrollToItemOnInsert(let autoScrollItem, _, _):
        guard
          let autoScrollIndexPath = self.storage.allContent.indexPath(
            for: autoScrollItem.identifier)
        else {
          fallthrough
        }

        let greaterIndexPath = max(autoScrollIndexPath, indexPath)
        visibleSlice = self.storage.allContent.sliceTo(
          indexPath: greaterIndexPath, plus: Content.Slice.defaultSize)

      case .none:

        visibleSlice = self.storage.allContent.sliceTo(
          indexPath: indexPath, plus: Content.Slice.defaultSize)
      }
    }

    let diff = ListView.diffWith(
      old: presentationState.sectionModels, new: visibleSlice.content.sections)
    ...
    self.performBatchUpdates(
      with: diff, animated: reason.animated, updateBackingData: updateBackingData
    ) { finished in
      self.updateVisibleItemsAndSections()
      callerCompletion(finished)
    }

    if case let AutoScrollAction.scrollToItemOnInsert(
      autoScrollItem, autoScrollPosition, animated) = self.autoScrollAction,
  ...
  private func preparePresentationStateForScroll(
    to toIndexPath: IndexPath, scroll: @escaping () -> Void
  ) -> Bool {
...
extension UIScrollView {
  ...
  fileprivate func isScrolledNearBottom() -> Bool {
  ...
  fileprivate func setAlwaysBounce(_ bounce: Bool, direction: LayoutDirection) {
    switch direction {
    case .vertical:
