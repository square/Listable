...
extension ListView {
  final class Delegate: NSObject, UICollectionViewDelegate, ListViewLayoutDelegate {
    ...
    private var displayedSupplementaryItems:
      [ObjectIdentifier: PresentationState.HeaderFooterViewStatePair] = [:]
    ...
    func collectionView(
      ...
      guard
        let headerFooter = self.displayedSupplementaryItems.removeValue(
          forKey: ObjectIdentifier(view))
      else {
        return
      }
