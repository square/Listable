...
public struct Snapshot<Iteration: SnapshotIteration> {
    ...
    internal typealias OnFail = (_ message: String, _ file: StaticString, _ line: UInt) -> Void
    ...
    public init(
        ...
        let allNames = iterations.map(\.name)
    ...
    public func test<OutputFormat: SnapshotOutputFormat>(
        ...
        for iteration in iterations {
            ...
            do {
                let rendering = iteration.prepare(render: try test(iteration))
                let data = try OutputFormat.snapshotData(with: rendering)
                onFailData = data

                let existingData = try existingData(at: url)
