...
class SnapshotTests: XCTestCase {
  fileprivate func newSnapshot(with test: @escaping Snapshot<TestIteration>.Test) -> Snapshot<
    TestIteration
  > {
  ...
  func test_image_and_text_output() {
    let snapshot = Snapshot(iterations: iOSDevice.allAvailable.toSnapshotIterations()) {
      iteration in
      ...
      for _ in 1...3 {
        let view = ViewType2(
          frame: .init(origin: .init(x: 10.0, y: 10.0), size: .init(width: 50.0, height: 50.0)))
        view.backgroundColor = .init(white: 1.0, alpha: 1.0)

        let view2 = ViewType3(
          frame: .init(origin: .init(x: 15.0, y: 15.0), size: .init(width: 30.0, height: 30.0)))
