...
class SnapshotTests: XCTestCase {
    ...
    {
        Snapshot(for: [TestIteration(name: "Test")], test: test)
    }
    ...
    func test_no_asset_writes_and_passes() {
        var snapshot = newSnapshot(with: { _ in
            "Result"
        })
    ...
    func test_image_and_text_output() {
        ...
        let snapshot = Snapshot(for: iterations) { _ in
...
private struct TestOutputFormat: SnapshotOutputFormat {
    ...
    static func snapshotData(with renderingFormat: String) throws -> Data {
        renderingFormat.data(using: .utf8)!
    }

    static var outputInfo: SnapshotOutputInfo {
        SnapshotOutputInfo(directoryName: "TestOutput", fileExtension: "test.txt")
    }
...
private struct TestIteration: SnapshotIteration {
...
private final class ViewType1: UIView {}
private final class ViewType2: UIView {}
private final class ViewType3: UIView {}
