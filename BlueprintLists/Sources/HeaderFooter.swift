...
///
/// A simple, non-tappable header that shows a label might look like this:
/// ```
...
/// 3) `elementRepresentation`
/// 2) `pressedBackground` (Only if the header/footer is pressed, eg if the wrapping `HeaderFooter` has an `onTap` handler.)
/// 1) `background`
///
