    ...
    return List { list in
        ...
        section += self.itemization.variations.all.map { variation in
          Item(
            ...
            selection: .isSelectable(
              isSelected: self.itemization.variations.selected.contains(variation)),
            onSelect: { _ in
      ...
      list += Section(identifier: SectionIdentifier.discounts) { section in
        ...
        section += self.availableOptions.allDiscounts.map { discount in
          ToggleItem(
            content: .init(
              title: discount.name, detail: "$0.00", isOn: self.itemization.has(discount))
          ) { isOn in
            if isOn {
      ...
      list += Section(identifier: SectionIdentifier.taxes) { section in
        ...
        section += self.availableOptions.allTaxes.map { tax in
          ToggleItem(
            content: .init(title: tax.name, detail: "$0.00", isOn: self.itemization.has(tax))
          ) { isOn in
            if isOn {
...
struct Header: BlueprintHeaderFooterElement, Equatable {
  ...
  var element: Element {
    return Inset(
      ...
      wrapping: Label(text: self.title) { label in
  ...
  func element(with info: ApplyItemElementInfo) -> Element {
    var box = Box(
      ...
      wrapping: Inset(
        ...
        wrapping: Row() { row in
          ...
          row.add(
            child: Label(text: self.title) { label in
              label.font = .systemFont(ofSize: 18.0, weight: .semibold)
            })
          row.add(
            child: Label(text: self.detail) { label in
    ...
    if info.state.isSelected {
      ...
      box.shadowStyle = .simple(
        radius: 2.0, opacity: 0.25, offset: .init(width: 0, height: 1.0), color: .black)
    } else {
    ...
    var box = Box(
      ...
      wrapping: Inset(
        ...
        wrapping: Row() { row in
          ...
          row.add(
            growPriority: 0.0,
            child: Label(text: self.content.title) { label in
              label.font = .systemFont(ofSize: 18.0, weight: .semibold)
            })
          ...
          row.add(
            growPriority: 0.0,
            child: Label(text: self.content.detail) { label in
              label.font = .systemFont(ofSize: 18.0, weight: .regular)
            })
          ...
          row.add(
            growPriority: 0.0, child: Toggle(isOn: self.content.isOn, onToggle: self.onToggle))
        })
    )
