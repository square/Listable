...
SIMULATOR_NAME="Listable CI iPhone 11 Pro Max (iOS 16.2)"

xcrun simctl delete "$SIMULATOR_NAME" || true

# Create New Simulators

DEVICE_UUID=$(xcrun simctl create "$SIMULATOR_NAME" "iPhone 11 Pro Max" "com.apple.CoreSimulator.SimRuntime.iOS-16-2")
echo "Created iOS 16.2 simulator ($SIMULATOR_NAME): $DEVICE_UUID"
